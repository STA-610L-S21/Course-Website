...
The full data actually includes data for more states but we will focus on just Minnesota like the Gelman and Hill book.
...
It is often convenient to stack our observations into a long vector $Y_{N \times 1}$ organized by groups.  

For simplicity in exposition, assume $n_j=n$ and the total sample size $N=nJ$.

Then assuming $Y$ follows a multivariate normal distribution (which follows from our specification previously), we can express
\begin{equation*}
...
The *Kronecker product* is a convenient way to express patterned covariance matrices (among other things).  For matrices $A_{m \times n}$ and $B_{p \times q}$, the *Kronecker product*
