...
Linear mixed effects models extend the standard linear models to allow for random effects. They overlap with hierarchical models in many ways.
...
When dealing with longitudinal data, mixed effecs models are especially convenient when there are no set times for observation of outcomes, which makes it challenging to estimate a covariance matrix across time in a multivariate setting.
...
We have already a bit of this with the random effect models we have seen so far.
...
Often, all observations may be treated as independent.
