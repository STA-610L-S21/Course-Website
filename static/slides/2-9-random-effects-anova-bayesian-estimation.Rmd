...
Bayesian estimation is often the approach of choice for fitting hierarchical models.
...
In addition to this data model, we will also need to specify a prior distribution for $(\mu,\tau^2,\sigma^2)$, which we will write as $p(\theta)=p(\mu,\tau^2,\sigma^2)$.
...
We will start with a .hlight[default semi-conjugate] prior specification given by
...
Using an $\mathcal{IG} \left(\dfrac{\eta_0}{2}, \dfrac{\eta_0\tau_0^2}{2}\right)$ distribution for $\tau^2$, we can now see that $\tau_0^2$ is somewhere in the "center" of the distribution (between the mode $\frac{\eta_0\tau_0^2}{\eta_0+2}$ and the mean $\frac{\eta_0\tau_0^2}{\eta_0-2}$).

As the "prior sample size" $\eta_0$ increases, the difference between these quantities goes to 0.
...
2. Prior distributions, specified in terms of prior guesses $(\mu_0,\tau_0^2,\sigma_0^2)$ and certainty/prior sample sizes $(\gamma^2_0,\eta_0,\nu_0)$
