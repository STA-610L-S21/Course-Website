...
subtitle: "Genealized linear mixed effects models (Part I)"
...
In the .hlight[generalized linear mixed effects model (GLMM)] for longitudinal data, we assume the conditional distribution of each $Y_{ij}$, conditional on ${\bf b}_i$, belongs to the exponential family with conditional mean
$$g(E[Y_{ij} \mid {\bf b}_i])={\bf X}_{ij}'\boldsymbol{\beta}+{\bf Z}_{ij}'{\bf b}_i,$$
where $g(\cdot)$ is a known link function.

Assume the ${\bf b}_i$ are independent across subjects with ${\bf b}_i \sim N({\bf 0}, {\bf D})$.
...
that is, we have random slopes and intercepts, then we can assume
...
so that $E(Y_{ij} \mid {\bf X}_{ij})={\bf X}_{ij}'\boldsymbol{\beta}$, when $g(\cdot)$ is non-linear (say the logit), then
...
Consider an unobserved continuous variable $W_{ij}$.
...
Note that some software packages require Laplace approximation with Gaussian quadrature if the number of random effects is more than 1 for the sake of computational efficiency.
